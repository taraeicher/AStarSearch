	...
	do
	{
		...
		else
		{
			...
			while(child != NULL)
			{
				...
				if(!duplicate) addNodeToFrontier(child, &frontier);
				child = child->rightSibling;
			}
		}
	}
	while(frontier.first !=NULL && !goalFound);
...
State getStartState()
{
	...
	printf("\nFirst, I will need your starting state. Tiles must contain single digits between 1 and 8 only.\n");
	...
	for(i = 1; i <= 3; i++)
	{
		...
		while(j <= 3)
		{
			...
			if(value != 10)
			{
				char *junk = (char*)malloc(sizeof(char));
				do *junk = getc(stdin); while(*junk != 10);	
				free(junk);
			}
...
State getGoalState()
{
	...
	printf("\nNow, I will need your goal state. Tiles must contain single digits between 1 and 8 only.\n");
	printf("For the blank space, enter 0.\n");
...
{
	return findValueInGrid(theState, 48);
}
...
void printGrid(State state)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		for(j = 0; j < 3; j++)
		{
			if((int)state.grid[i][j] == 48) printf("  ");
